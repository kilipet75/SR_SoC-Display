...
#define AMOLED_143_V2 //auskommentieren bei Display V1
...
void lcd_lvgl_Init(void) {
  ...
  lv_init();
  lv_color_t *buf1 = heap_caps_malloc(EXAMPLE_LCD_H_RES * EXAMPLE_LVGL_BUF_HEIGHT * sizeof(lv_color_t), MALLOC_CAP_DMA);  //MALLOC_CAP_DMA, MALLOC_CAP_SPIRAM
  assert(buf1);
  lv_color_t *buf2 = heap_caps_malloc(EXAMPLE_LCD_H_RES * EXAMPLE_LVGL_BUF_HEIGHT * sizeof(lv_color_t), MALLOC_CAP_DMA);  //MALLOC_CAP_DMA, MALLOC_CAP_SPIRAM
  assert(buf2);
...
static void example_lvgl_port_task(void *arg) {
  ...
  vTaskDelay(pdMS_TO_TICKS(10)); //warten bis semaphore vollständig erstellt
